...
{
// Версия прошивки
constexpr const char* VERSION = "1.0.6"; // Обновите версию при каждом изменении прошивки
...
inline void load() {
    ...
    SOFT_START_STEP_MS = prefs.getUInt("SS_STEP_MS", SOFT_START_STEP_MS);
    ...
    overcurrentTimeoutMs = prefs.getUInt("OVER_TO_MS", overcurrentTimeoutMs);

    MAX_FORWARD_TIME_MS = prefs.getUInt("MAX_FWD_MS", MAX_FORWARD_TIME_MS);
    MAX_REVERSE_TIME_MS = prefs.getUInt("MAX_REV_MS", MAX_REVERSE_TIME_MS);
    FORWARD_LIMIT_RUN_ON_MS = prefs.getUInt("FWD_RUN_ON", FORWARD_LIMIT_RUN_ON_MS);
    REVERSE_LIMIT_RUN_ON_MS = prefs.getUInt("REV_RUN_ON", REVERSE_LIMIT_RUN_ON_MS);
    MAX_LIFT_ENCODER_TICKS = prefs.getUInt("MAX_ENCODER", MAX_LIFT_ENCODER_TICKS);

    IR_CODE_UP = prefs.getUInt("IR_UP", IR_CODE_UP);
    IR_CODE_DOWN = prefs.getUInt("IR_DOWN", IR_CODE_DOWN);
    IR_CODE_STOP = prefs.getUInt("IR_STOP", IR_CODE_STOP);
    IR_CODE_REPEAT = prefs.getUInt("IR_REP", IR_CODE_REPEAT);

    String savedOta = prefs.getString("OTA_URL", otaUrl);
    snprintf(otaUrl, sizeof(otaUrl), "%s", savedOta.c_str());
    otaUpdateIntervalMs = prefs.getUInt("OTA_INT", otaUpdateIntervalMs);
...
inline void save() {
    ...
    prefs.putUInt("SS_STEP_MS", SOFT_START_STEP_MS);
    ...
    prefs.putUInt("OVER_TO_MS", overcurrentTimeoutMs);

    prefs.putUInt("MAX_FWD_MS", MAX_FORWARD_TIME_MS);
    prefs.putUInt("MAX_REV_MS", MAX_REVERSE_TIME_MS);
    prefs.putUInt("FWD_RUN_ON", FORWARD_LIMIT_RUN_ON_MS);
    prefs.putUInt("REV_RUN_ON", REVERSE_LIMIT_RUN_ON_MS);
    prefs.putUInt("MAX_ENCODER", MAX_LIFT_ENCODER_TICKS);

    prefs.putUInt("IR_UP", IR_CODE_UP);
    prefs.putUInt("IR_DOWN", IR_CODE_DOWN);
    prefs.putUInt("IR_STOP", IR_CODE_STOP);
    prefs.putUInt("IR_REP", IR_CODE_REPEAT);

    prefs.putString("OTA_URL", otaUrl);
    prefs.putUInt("OTA_INT", otaUpdateIntervalMs);
